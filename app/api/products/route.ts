...
function normalizeMediaUrl(url: string): string {
  ...
  // Convert old raw GitHub URLs to proxy URLs so they render
  // even when the repository is private.
  const rawMatch = url.match(/raw\.githubusercontent\.com\/[^/]+\/[^/]+\/[^/]+\/(.+)$/)
  if (rawMatch?.[1]) {
    return `/api/images/${rawMatch[1]}`
  }

  return url
}

function normalizeProductMedia(product: any): any {
  return {
    ...product,
    images: Array.isArray(product.images) ? product.images.map((img: string) => normalizeMediaUrl(img)) : product.images,
    video: typeof product.video === 'string' ? normalizeMediaUrl(product.video) : product.video,
    colorVariants: Array.isArray(product.colorVariants)
      ? product.colorVariants.map((variant: any) => ({
          ...variant,
          images: Array.isArray(variant.images)
            ? variant.images.map((img: string) => normalizeMediaUrl(img))
            : variant.images,
        }))
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (category) {
      ...
      return NextResponse.json({ products: filtered.map(normalizeProductMedia) })
    }

    return NextResponse.json({ products: products.map(normalizeProductMedia) })
  } catch (error) {
